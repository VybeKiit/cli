...
const GATE_ORG = process.env.VYBEKIIT_GATE_ORG ?? 'VybeKiit';
/** Private delivery repos; access to ANY one proves a paid license. Override CSV with VYBEKIIT_GATE_REPOS. */
const GATE_REPOS = (process.env.VYBEKIIT_GATE_REPOS ?? 'web,mobile,extension')
  .split(',')
  .map((name) => name.trim())
  .filter((name) => name.length > 0);
/** Where to send an ungated buyer to purchase. Override with VYBEKIIT_SITE_URL. */
const SITE_URL = process.env.VYBEKIIT_SITE_URL ?? 'https://vybekiit.com';
...
function ghSucceeds(args: readonly string[]): boolean {
  return spawnSync('gh', [...args], { stdio: 'ignore' }).status === 0;
}

/** True when the signed-in user is an active member of {@link GATE_ORG}. */
function isOrgMember(): boolean {
 ...
 */
export function checkAccess(): GateResult {
...
/** Plain-language guidance for a blocked run (never a raw error). */
 ...
 */
export function ensureAccessOrExit(): boolean {
