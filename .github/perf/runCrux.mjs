...
function parseArgs(argv) {
  ...
  for (const arg of argv) {
    ...
    else if (arg.startsWith('--form-factor='))
      args.formFactor = arg.slice('--form-factor='.length).toUpperCase();
    ...
    else if (args.url) throw new Error(`Unexpected argument: ${arg}`);
    else args.url = arg;
  }
...
async function main() {
  ...
  let result = await queryCrux(
    args.origin ? { origin: args.url } : { url: args.url },
    args.formFactor,
    args.key,
  );
  ...
  if (result.notFound) {
    ...
    console.log(
      args.json ? JSON.stringify({ url: args.url, noFieldData: true, message }) : message,
    );
