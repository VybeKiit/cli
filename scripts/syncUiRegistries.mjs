...
const MANIFEST_PATH = join(REPO_ROOT, 'scripts/ui-registry-manifest.json');
const LOCK_PATH = join(REPO_ROOT, 'scripts/ui-registry-lock.json');
  ...
  for (const url of source.urls ?? []) {
    try {
      ...
      results.push({
        ...
        tags: source.tags ?? [],
        portable: isPortable(item.dependencies ?? []),
        category: inferCategory(String(item.name ?? ''), item),
      });
    } catch (error) {
 ...
 * @param {Record<string, unknown>} item
 */
function inferCategory(name, item) {
  const haystack = `${name} ${item.title ?? ''}`.toLowerCase();
  if (haystack.includes('hero')) return 'hero';
  if (haystack.includes('pricing')) return 'pricing';
  if (haystack.includes('chart') || haystack.includes('table')) return 'data-display';
  if (haystack.includes('form') || haystack.includes('input')) return 'form';
  if (haystack.includes('nav') || haystack.includes('sidebar')) return 'navigation';
  if (haystack.includes('background') || haystack.includes('beam')) return 'background';
  return 'component';
}
  ...
  for (const slug of slice) {
    try {
      ...
      results.push({
        ...
        paths: [`${installPath}/${basename(slug)}`],
        dependencies: [],
