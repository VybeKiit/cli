...
const PACKAGES = [
  ...
  },
  {
    ...
    body: 'Playwright automation for Chrome Web Store Developer Console actions the upload API does not cover: listing updates, screenshots, review status, new store items.',
    exports:
      '`publish`, `submitForReview`, `readListingState`, `CWS_AUTOMATION_VERBS`, `connectToCwsChrome`',
    config: 'Dedicated Chrome profile at `$HOME/.cws-chrome-profile` — not env-based.',
    skills: 'Extension template publish skills, `refresh-cws-listing`',
    inScope: 'CWS verb allowlist, safe click guards, listing schema.',
    outScope: 'Not local extension builds — use template build scripts.',
  },
  {
    name: 'report-mode',
    tagline:
