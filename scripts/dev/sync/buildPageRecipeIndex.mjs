...
const validateSourceFile = async ({ repoRoot, recipe }) => {
  ...
  return { ...recipe, sourceCode, todos };
};
...
export const validatePageRecipeManifest = async ({ repoRoot = REPO_ROOT, manifest }) => {
  ...
  for (const group of manifest.groups) {
    ...
    for (const recipe of group.recipes) {
      ...
      recipes.push(await validateSourceFile({ repoRoot, recipe }));
    }
...
export const buildGeneratedPageRecipeComponentsSource = (index) => {
  ...
  const imports = recipes
    .map(
      (recipe) =>
        `import { ${recipe.exportName} as ${recipe.exportName}Recipe } from '${importPathFromDataModule(recipe.sourcePath)}';`,
    )
    .join('\n');
  const entries = recipes
    .map((recipe) => `  '${recipe.slug}': ${recipe.exportName}Recipe,`)
    .join('\n');
...
export const buildPageRecipeIndex = async ({ repoRoot = REPO_ROOT, check = false } = {}) => {
  ...
  console.log(`Wrote ${OUT_PATH} and ${OUT_COMPONENTS_PATH} (${index.groups.length} groups)`);
};
