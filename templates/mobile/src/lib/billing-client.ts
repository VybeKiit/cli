 ...
 *
 * Ships as a marked stub so the pricing screen renders and the app builds with no
 ...
 * so switching Lemon Squeezy / Stripe / PayPal never touches the UI. Mirrors the
 * web template's `billing-client.ts` exactly so both platforms behave identically.
 */
