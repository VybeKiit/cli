...
- They describe something big and vague ("a fitness app", "like Uber for dogs") without specifics.
...
2. **Walk the decision tree — one question at a time.**
   ...
   - When they use a fuzzy word ("account", "user", "booking"), sharpen it: *"Do you mean someone who
     pays, or someone who just opens the app?"*
   - When a term is resolved, update `CONTEXT.md` immediately (see `planning-vybekiit.md`).
   - Remember: this is a **phone app** — saved data, sign-in, and payments live on their **web
     backend**; clarify what runs on the phone vs what the backend handles.
...
4. **Close with a plain summary.**
   ...
   `goal-index.md` (publish to stores, connect sign-in, take money, etc.).
