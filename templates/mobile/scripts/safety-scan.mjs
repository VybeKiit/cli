...
// Match config files: "vite.config.ts" -> match, "main.tsx" -> no match.
const CONFIG_FILE_PATTERN = /\.config\.[cm]?[jt]s$/;
const isConfigFile = (name) => CONFIG_FILE_PATTERN.test(name);
...
const leakPathsForSecret = (name, line, abs) => {
  ...
  if (isEnvFile(name)) {
    ...
    const shipsToClient = isPublicName(line) || (PROFILE.everythingShips && !isConfigFile(name));
    if (shipsToClient) {
      addLeakPath(paths, 'public');
    }
  }
