#!/usr/bin/env node
/**
 * VybeKiit dev launcher for the extension (WXT) template — the uniform `pnpm dev`
 * entry. Run `pnpm dev --help` to see every mode. Self-contained so it keeps
 * working after the template is scaffolded into a buyer's repo.
 */
import { spawn } from 'node:child_process';
import process from 'node:process';

const TEMPLATE = 'extension (WXT)';

/** Ordered modes. `flag` selects the mode; `script` is the package.json script it runs. */
const MODES = [
  { flag: '--firefox', script: 'dev:firefox', desc: 'Firefox dev build (wxt -b firefox)' },
];

/** Runs when no mode flag is passed (Chrome). */
const DEFAULT_SCRIPT = 'dev:wxt';
...
/**
 * Print the mode list and examples.
 * @returns {void}
 */
function printHelp() {
  const lines = [
  ...
  lines.push(`  ${'(default)'.padEnd(11)} Runs \`${DEFAULT_SCRIPT}\` (Chrome)`);
  lines.push('', 'Examples:', '  pnpm dev', '  pnpm dev --firefox');
