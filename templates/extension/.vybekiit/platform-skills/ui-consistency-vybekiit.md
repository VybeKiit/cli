...
1. **Primitive-first** — `Button`, `Input`, `Card`, `Alert`, `Badge`, `Separator`, `Skeleton`, `Avatar`, `Tabs`, `Dialog`, `Sheet`, `Select`, `DropdownMenu`, `Sonner` from `src/components/ui/`; popup ships the core subset (`Button`, `Input`, `Card`, `Alert`)
