---
name: add-images
description: add extension icons and static images that ship with the extension package. Use when the builder says something like: add my logo; hero image; app icon; add images.
metadata:
...
> `scripts/optimizeAssets.mjs` to emit 16/48/128 PNGs referenced in `wxt.config.ts`. Extensions
...
2. **Optimize.** Run `node scripts/optimizeAssets.mjs` or `pnpm build`.
