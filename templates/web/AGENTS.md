...
- **Decide everything technical yourself.** Pick the library, the schema, the structure, the
  deploy target. Don't ask the vibe coder to choose between technical options — choose for them and
  briefly say what you did in plain words.
- **Guide the few manual steps.** When only the vibe coder can do something (paste a key from a
...
2. **One action at a time.** Do a step, then **verify it worked before moving on** (the skills call
   this *verify-before-advance* — it's what stops a vibe coder getting silently stuck and asking for
   a refund).
3. **Translate every error** into "what happened + the one thing to do about it." Never paste a raw
   stack trace at the vibe coder.
...
3. If they **decline** → build anyway. Impatient vibe coders stay in control.
...
- The kit's logic lives in `@vybekiit/*` packages (payments, accounts, data). **Use them — don't
  reinvent them.** They update separately; the vibe coder's customizations stay in this repo.
- All secret settings live in **one** file: `.env` (documented by `.env.example`). One source of
  ...
  `.env.example` only. If the vibe coder pastes a secret in chat, warn them and have them paste into
...
- Pick blocks from `.vybekiit/agent/ui-sources.md` (shadcn, Magic UI, Kokonut, 21st.dev, etc.) — the vibe coder never chooses.
...
`src/lib/auth-client.ts`, checkout in `src/lib/billing-client.ts`. Never show the vibe coder a marker or
