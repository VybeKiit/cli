#!/usr/bin/env node
/**
 * VybeKiit dev launcher for the web (Next.js) template — the uniform `pnpm dev`
 * entry. Run `pnpm dev --help` to see what it runs. Self-contained so it keeps
 * working after the template is scaffolded into a buyer's repo.
 */
import { spawn } from 'node:child_process';
import process from 'node:process';

const TEMPLATE = 'web (Next.js)';

/** Ordered modes. `flag` selects the mode; `script` is the package.json script it runs. */
const MODES = [];

/** Runs when no mode flag is passed. */
const DEFAULT_SCRIPT = 'dev:next';
...
function printHelp() {
  ...
  lines.push('', 'Examples:', '  pnpm dev', '  pnpm dev --port 4000');
