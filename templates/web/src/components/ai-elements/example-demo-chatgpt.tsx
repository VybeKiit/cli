...
const SuggestionItem = memo(
  ({ icon: Icon, text, color, onSuggestionClick }: SuggestionItemProps) => {
    const handleClick = useCallback(() => onSuggestionClick(text), [onSuggestionClick, text]);

    return (
      <Suggestion className="font-normal text-foreground" onClick={handleClick} suggestion={text}>
...
const Example = () => {
  ...
  const handleUploadFile = useCallback(() => handleFileAction('upload-file'), []);
  const handleUploadPhoto = useCallback(() => handleFileAction('upload-photo'), []);
  const handleTakeScreenshot = useCallback(() => handleFileAction('take-screenshot'), []);
  const handleTakePhoto = useCallback(() => handleFileAction('take-photo'), []);
  const handleToggleWebSearch = useCallback(() => setUseWebSearch((prev) => !prev), []);
  const handleToggleMicrophone = useCallback(() => setUseMicrophone((prev) => !prev), []);
