...
function WindowControls({ variant = 'macos', headerStyle = 'full' }: WindowControlsProps) {
  ...
  if (variant === 'macos' || variant === 'safari') {
    ...
    return (
      <div className="flex gap-2">
        ...
        >
          {headerStyle !== 'minimal' && (
            <div className="w-1.5 h-0.5 bg-red-900/60 opacity-0 group-hover:opacity-100 transition-opacity" />
          )}
        </div>
        <div
          className={cn(
        ...
        >
          {headerStyle !== 'minimal' && (
            <div className="w-1.5 h-0.5 bg-yellow-900/60 opacity-0 group-hover:opacity-100 transition-opacity" />
          )}
        </div>
        <div
          className={cn(
        ...
        >
          {headerStyle !== 'minimal' && (
            <div className="w-1 h-1 border border-green-900/60 opacity-0 group-hover:opacity-100 transition-opacity" />
          )}
        </div>
      </div>
    );
    ...
    return (
      <div className="flex gap-1">
        <div className="w-6 h-4 bg-muted/50 hover:bg-muted transition-colors cursor-pointer flex items-center justify-center">
          <div className="w-2 h-0.5 bg-foreground/60" />
        </div>
        <div className="w-6 h-4 bg-muted/50 hover:bg-muted transition-colors cursor-pointer flex items-center justify-center">
          <div className="w-2 h-2 border border-foreground/60" />
        </div>
        <div className="w-6 h-4 bg-red-500/80 hover:bg-red-500 transition-colors cursor-pointer flex items-center justify-center">
          <div className="w-2 h-0.5 bg-white rotate-45" />
  ...
  return (
    <div className="flex gap-1.5">
      <div className={`${sizeClasses} rounded-full border border-foreground/20 bg-foreground/10`} />
      <div className={`${sizeClasses} rounded-full border border-foreground/20 bg-foreground/10`} />
      <div className={`${sizeClasses} rounded-full border border-foreground/20 bg-foreground/10`} />
    </div>
  );
