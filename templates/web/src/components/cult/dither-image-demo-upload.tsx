...
function needsUnoptimizedImage(src: string): boolean {
  return src.startsWith('blob:') || src.startsWith('data:') || REMOTE_HTTP_RE.test(src);
}
...
const QUICK_GHIBLI: { href: string; label: string }[] = Array.from({ length: 9 }, (_, i) => {
  const n = i + 1;
  return {
...
function CaptionMetaChip({ kicker, children }: { kicker: string; children: ReactNode }) {
  return (
    <span
      className={cn(
    ...
    >
      ...
      <span className="min-w-0 font-medium text-foreground tabular-nums">{children}</span>
    </span>
  );
}
...
export default function DitherImageDemoUpload({
  ...
  const [aspect, setAspect] = useState<(typeof ASPECT_OPTIONS)[number]['value']>('square');
            ...
            >
              <div className="flex min-h-0 flex-1 flex-col gap-3">
                <div className="shrink-0 flex flex-col gap-1.5">
                  <Label className="text-foreground text-xs" htmlFor={`${baseId}-url`}>
                    Image or GIF URL
                  </Label>
                ...
                <div className="shrink-0 flex flex-col gap-1.5">
                  <span className="text-foreground text-xs">Ghibli stills (local)</span>
                  <div className="flex flex-wrap gap-2">
                    {QUICK_GHIBLI.map((g) => (
                      <Button
                        className={BTN_PRESS}
                ...
                <div className="flex min-h-0 flex-1 flex-col gap-2 border-border/60 border-t pt-3">
                  ...
                  <label
                    className={cn(
                      ...
                      fileDragging && 'border-primary bg-primary/8 hover:border-primary',
                    )}
                    htmlFor={`${baseId}-file`}
                ...
                <div className="shrink-0 pt-2">
                  <Button className={cn('w-full', BTN_PRESS)} onClick={resetSliders} type="button">
                    Reset settings
                  </Button>
                </div>
              </div>
            ...
            <div className="flex flex-col gap-8">
                ...
                <div className="flex flex-col gap-4">
                  <div className="flex flex-wrap items-center justify-between gap-3 border-border/60 border-b pb-4">
                    <div className="flex flex-col gap-0.5">
                      <Label className="text-foreground text-xs" htmlFor={`${baseId}-reveal`}>
                        Partial reveal
                      </Label>
                  ...
                  >
                    <div className="flex flex-col gap-1.5">
                      <Label className="text-foreground text-xs" htmlFor={`${baseId}-reveal-dir`}>
                        Mask direction
                      </Label>
                      <select
                        ...
                        onChange={(e) => setRevealDir(e.target.value as DitherRevealDirection)}
                        value={revealDir}
                      >
              ...
              >
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="flex flex-col gap-1.5">
                    <Label className="text-foreground text-xs" htmlFor={`${baseId}-size`}>
                      Cell size
                    </Label>
                    <select
                      className={SELECT_SURFACE}
                  ...
                  <div className="flex flex-col gap-1.5">
                    <Label className="text-foreground text-xs" htmlFor={`${baseId}-aspect`}>
                      Aspect ratio
                    </Label>
                    <select
                      className={SELECT_SURFACE}
                      id={`${baseId}-aspect`}
                      onChange={(e) =>
                        setAspect(e.target.value as (typeof ASPECT_OPTIONS)[number]['value'])
                      }
                      value={aspect}
                    >
