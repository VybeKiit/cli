  ...
  const renderTaskRow = (task: Task) => {
    ...
    return (
      <TableRow key={task.id} className="hover:bg-muted/50">
        ...
        <TableCell className="h-16 px-4 text-sm text-muted-foreground">{task.assignee}</TableCell>
        <TableCell className="h-16 px-4">{getStatusBadge(task.status)}</TableCell>

        <TableCell className="h-16 px-4 text-sm text-muted-foreground">{task.dueDate}</TableCell>
