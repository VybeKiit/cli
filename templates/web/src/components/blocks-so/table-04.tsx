  ...
  return (
    <div className="flex -space-x-2">
      {people.map((person, index) => (
        ...
        >
          <AvatarFallback className={cn('font-medium text-white', getAvatarColor(person.initials))}>
            {person.initials}
          </AvatarFallback>
        </Avatar>
      ))}
...
function StatusBadge({ status, variant = 'default' }: { status: string; variant?: StatusVariant }) {
