  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h3 className="text-balance text-lg font-semibold text-foreground">Product Inventory</h3>
          <p className="text-pretty mt-1 text-sm text-muted-foreground">
            Track and manage product stock levels across all warehouse locations.
          </p>
        </div>
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-b hover:bg-transparent">
              ...
              <TableHead className="h-12 px-4 font-medium">Product Name</TableHead>
              <TableHead className="h-12 px-4 font-medium">Category</TableHead>
              <TableHead className="h-12 px-4 font-medium">Status</TableHead>
              <TableHead className="h-12 px-4 text-right font-medium">Unit Price</TableHead>
              <TableHead className="h-12 px-4 text-right font-medium">Last Restocked</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {filteredData.length > 0 ? (
              filteredData.map((item) => (
                <TableRow key={item.sku} className="hover:bg-muted/50">
                  ...
                  <TableCell className="h-14 px-4 font-medium">{item.productName}</TableCell>
                  <TableCell className="h-14 px-4 text-sm text-muted-foreground">
                    {item.category}
                  </TableCell>
                  <TableCell className="h-14 px-4">{getStatusBadge(item.status)}</TableCell>
