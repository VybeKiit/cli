...
const PoseCard = ({
  ...
  const label = meta === undefined ? pose : meta.label;
...
/**
 * Gallery grid for all assistant mark poses, sizes, and demo scenes.
 *
 * @returns Showcase view for the component library.
 * @example
 * <BuilderAssistantMarkShowcase />;
 */
export const BuilderAssistantMarkShowcase = () => (
  <div className="mx-auto flex w-full max-w-6xl flex-col gap-10 p-6 md:p-10">
    <header className="space-y-2 text-center">
      ...
      <p className="mx-auto max-w-xl text-muted-foreground text-sm">
        Cursor, Claude Code, and Codex marks — {CLAUDE_OCTOPUS_POSES.length} animated octopus poses
        and a six-step size scale from XS to XXL.
      </p>
    </header>
    ...
    <section className="space-y-4">
      ...
      <p className="max-w-2xl text-muted-foreground text-sm">
        ...
        tween). Every rect coordinate is a whole number, so edges land on exact device pixels at any
        size. All {CLAUDE_OCTOPUS_POSES.length} poses now render on the grid — each composed from
        shared body parts, an eye state, and a pixel-art prop. A sample below; the full set is in
        the grid.
      </p>
