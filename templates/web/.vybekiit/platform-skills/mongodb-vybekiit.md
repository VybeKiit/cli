...
**Agent-only.** Not invoked by buyer `save-data`. Use when `.env` already has `DATA_PROVIDER=mongodb`.
...
2. Data access via `@vybekiit/db` → `resolveDataProvider()` — never direct driver calls in UI
3. Auth: `better-auth` may use Mongo adapter when configured — see `better-auth-vybekiit.md`
4. Atlas CLI installed by `vybekiit doctor` when `DATA_PROVIDER=mongodb`
...
MongoDB, NoSQL, collection, document, BSON, Atlas (use plain language from `language.md`)
