...
4. **Pre-push only** — buyer templates ship an identical husky **pre-push** hook (not pre-commit). The builder never pushes; the agent does. The hook runs `pnpm verify` before push succeeds.

## Verify

`pnpm lint` exits 0 (warnings OK). Full smoke: `pnpm verify`.
