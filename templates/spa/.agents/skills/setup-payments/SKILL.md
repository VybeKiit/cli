...
2. **Connect taking money (Live work first).** Prefer the shared Live work path (ADR-0039):
   ...
   - On success, read the JSON `buyerMessage` out loud. Pin keys are already written (`pinKeys` —
     never print values).
   - If it fails with missing credentials / ladder exhausted, collect keys via the steps below, then
     re-run Live work payments so pin stays in the package runner.
...
4. **Stripe (opt-in).** Merge `mcp-stripe.json` via `agent/mcp-setup.md`; builder OAuth once.
   ...
   Re-run Live work payments (step 2) when Stripe is the winning rail.
...
5. **PayPal (opt-in).** Merge `mcp-paypal.json` via `agent/mcp-setup.md`; builder OAuth once (sandbox).
   ...
   Re-run Live work payments (step 2) when PayPal is the winning rail.
...
- Never reimplement preference ladder / hop in this skill — always `vybekiit live-work payments` first.
