...
> (Agent-only) Run `vybekiit plan-data-model entities.json [provider]` — do not eval TypeScript from
> node_modules. Never say primary key, foreign key, migration, schema, or normalization to the builder.
...
Single trivial entity (e.g. one list of notes) — you may run `vybekiit plan-data-model` inline inside `save-data` step 2
