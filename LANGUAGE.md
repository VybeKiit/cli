...
**Preference ladder** — the ordered list of adapters the kit will try for one concern when the
builder did not name a vendor. Lives in MAINTAINED packages; consumed by local-dev Live work and
buyer skills. Default data: supabase → neon → railway. Default host: cloudflare → render →
railway → vercel. Default payments: lemon-squeezy → stripe → paypal. Auth has no ladder — it
follows the chosen data store.
_Avoid_: silent fallback, default map, free-tier auto-switch (say hop only when classified).

**Free-tier hop** — switching to the next entry on a **Preference ladder** only when the active
adapter fails with a plan/quota/free-tier limit **or** an onboarding-blocked free path (e.g. store
papers not ready) — not wrong keys, not network, not a named vendor. After a successful hop, the
winner is pinned into `*_PROVIDER` env and the checklist Decision log. Combo work hops per concern;
partial creates get best-effort orphan cleanup on the loser.
_Avoid_: any-error hop, silent hop without pin.

**Named vendor stick** — when the builder (or prompt) names a vendor, try only that adapter; on
quota, stop and guide — never hop away from a named choice.

**Live work** — real provision / verify / cleanup of a concern (data, auth, payments, host, CRUD)
driven from the local-dev console or buyer skills, emitting the same tool events the journey rail
shows. Not the fixture animation path.
_Avoid_: demo progress, fake tool stream (those are test doubles only).

**Execution cascade** — when more than one path can perform Live work, try in order: MAINTAINED
runner → `vybekiit` CLI verb → official MCP → agent freeform. All paths normalize to the same
journey tool events.

**Mode-scoped teardown** — demo/dogfood Live work may create then delete; buyer production skills
configure only and never auto-delete unless the builder asks to remove.

**Dogfood apex** — the env-selected domain (`DOGFOOD_APEX`) used only for Live work / dogfood
subdomains (pool: yosefhayimsabag.dev, askally.io, ohadbaher.net). Never the production apex site.
_Avoid_: using apex roots for throwaway deploys.

**Env pin** — writing the ladder winner into root `.env` (provider id + provisioned secrets) plus
checklist Decision log, with secrets never shown in console output. Root `.env.example` documents
the full multi-adapter key matrix.
...
**Primitive story** — the live, source-derived showcase of one `@vybekiit/ui` primitive in the
component library: a controls-driven playground plus a collapsible live "show all" of every real
variant, size, and state. Variant/size options derive from the primitive's `cva` config; behavioral
primitives (Dialog, Select, Table…) add a hand-authored live override under
`apps/componentLibrary/src/stories/vybekiit/`. Generated by
`scripts/dev/sync/buildDesignSystemIndex.mjs` into `apps/componentLibrary/src/data/designSystem.ts`,
checked by `scripts/dev/checks/checkComponentStories.mjs` (ADR-0041).
_Avoid_: demo (reserved for a single third-party preview), Storybook story (no Storybook here),
variant card (static mockups are banned — a primitive story is live).

**Design system surface** — the dedicated area of the component library that lists only the
`@vybekiit/ui` primitives, grouped by family (Primitives, Forms, Overlays, Data display, Feedback…),
separate from the third-party block catalog and the **Page recipe catalog**. Each entry opens a
sectioned **Primitive story**.
_Avoid_: calling the whole gallery "the design system" (it also hosts third-party blocks and page
recipes).

**Canonical component states** — the one shared state vocabulary used by both **Primitive stories**
and **Page recipes**: `default`, `disabled`, `loading`, `error`, `success`, `empty`, plus
control-specific `readonly` and `selected`. A recipe advertises the subset it enters via
`statesCovered`; a primitive declares the subset it demonstrates; both validate against
`apps/componentLibrary/src/lib/componentStates.ts`.
_Avoid_: ad-hoc per-component state names; using "status" as a synonym for the whole set.
...
**Safety scan** — the readable, zero-dependency go-live check (`scripts/safety-scan.mjs`) that reads
the project locally and reports leaks ordered easiest-to-spot to hardest across five rungs (secrets,
config, dependencies, sign-in & access, risky code). Owned template code, not a package, so the vibe
coder can read the check itself (ADR-0030). In buyer voice: "the safety check". Run by `check-safety`.
_Avoid_: calling it a scanner, SAST, or audit tool to the vibe coder; naming it a `@vybekiit/*` package.

**Leak path** — one of the seven ways a single secret can escape: code, git history, public prefix,
client bundle, api reply, logs, source map. The scan checks each independently so the report can say
in plain words which are true. A key in an untracked `.env` is the correct place — not a leak.

**Finding** — one result from the safety scan, carrying eight fields (id · rung · severity · detect ·
leakPaths · proof · plain · kitPreventable) with two faces: the engineer face (deterministic) and the
vibe-coder face (plain, localized title → what could happen → the one fix → traffic light). Buyer
hears only the plain face.

**Safety delta** — the gap between a kit-guided app and an untouched vibe-coded one, measured by
`kitPreventable` findings the guided build never ships. Doubles as the marketing signal and a
regression alarm if a template starts shipping a preventable finding.

**Trust artifact** — the principle that the verdict must come from something the vibe coder can read
and run (the plain script + catalog), never from an agent asserting "it's safe". Why the scan is
owned, readable, and zero-dependency rather than a compiled package.

**Defense-in-depth** — the second half of a finding's fix: after closing the leak, set a limit so a
future slip stays small (OpenAI/Anthropic spend cap, AWS billing alert, Stripe restricted key,
database password rotation). In buyer voice: "extra safety".

**Endpoint contract test** — the dep-free loopback e2e that boots the backend on an ephemeral port
(`app.listen(0)`) and drives every generated route with built-in `fetch`, decoding each response
against its registry Schema (see **API contract**). Runs on the **local dev adapter** in the required
gate (deterministic, no secrets) and proves the provider swap via injected-fake adapter tests; the
real cross-provider matrix runs nightly + `workflow_dispatch`, secret-gated. See ADR-0042.
_Avoid_: putting real provider secrets on the required gate; asserting out-of-band codes without a
provider admin/test API.
...
extension needs sign-in, payments, or data without a full marketing website. Its verifiable auth
surface is password + email-OTP + magic-link + password-reset + SMS-OTP plus the session-cookie /
bearer guard; social OAuth ("Sign in with Google") is a **client**-template concern via the provider
(ADR-0024), never an Express route. Verified per ADR-0042.
...
**Kit changelog** — the auto-generated record of what each Kit release contains: root `CHANGELOG.md`
(Keep a Changelog, version-first, area-tagged) plus the `/changelog` maintainer board in
`apps/componentLibrary`, both emitted by `scripts/dev/changelog/generateChangelog.mjs` from git
commits + filtered GitHub Issues. Maintainer-facing; it backs the Release line. Status marks reuse the
live-work legend (✅🔄⬜🅿️🧪💻). See ADR-0041.
_Avoid_: hand-editing `CHANGELOG.md` (it is generated); confusing it with the ChangelogPage recipe.

**ChangelogPage recipe** — a buyer-facing **surface recipe** (`pageRecipes/ChangelogPage.tsx`, demoed
at `/pages/changelog`): a product-changelog UI a buyer drops into their own app. Not the kit's own
release record.
_Avoid_: treating it as the Kit changelog.
...
**API contract** — the Effect-`Schema` route registry in `@vybekiit/core/http` that is the SSOT for
every generated endpoint: each route registers request/response/error Schemas, and both the OpenAPI
3.1 document and the typed frontend client are *derived* from it (no generator dependency). One
registry backs `http/express.ts` (backend) and `http/next.ts` (web). Extends the **JSON client** and
**HTTP outcome**; kills client/server drift (`getJson<User>` becomes a derived type). See ADR-0043.
_Avoid_: hand-writing the OpenAPI doc or a parallel client; treating the emitted artifacts as SSOT
(the registry is).
...
upstream registries; lock file at `scripts/data/ui-registry-lock.json`.

**SaaS showcase bundle** — curated subset of mirrored UI blocks (`scripts/data/saas-showcase-manifest.json`)
