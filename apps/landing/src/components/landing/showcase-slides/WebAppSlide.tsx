 ...
 *
 * @returns The rendered WebAppSlide element.
 * @example
 * ```tsx
 * <WebAppSlide />
 * ```
  ...
  <MiniBrowserChrome className="h-full" dark={true} url="saasflow.app">
    <div className="flex h-full flex-col bg-[#070b12]">
      <div className="flex items-center gap-1.5 border-white/6 border-b px-3 py-2">
        <span className="flex shrink-0 items-center gap-1.5 font-semibold text-xs text-white">
          <span className="h-3 w-3 rounded-sm bg-gradient-to-br from-[#62a1ff] to-[#1e6bff]" />
          SaaSFlow
        </span>
        <nav className="ms-1.5 hidden gap-2 text-xs text-white/45 md:flex">
          {NAV.map((item) => (
            <span key={item}>{item}</span>
          ))}
        </nav>
        <span className="ms-auto flex shrink-0 items-center gap-2">
          <span className="whitespace-nowrap text-xs text-white/55">Sign In</span>
          <Button className="h-6 rounded-md px-2 text-xs" size="sm">
            Get Started
          </Button>
        </span>
      </div>
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-4 text-center">
        <p className="font-bold text-xl text-white leading-[1.15]">The SaaS Starter Kit</p>
        <p className="font-bold text-xl leading-[1.15]">
          <span className="text-blue-gradient">for ambitious developers.</span>
        </p>
        <p className="mt-2.5 max-w-[210px] text-xs text-white/50 leading-relaxed">
          Ship faster with our production-ready starter kit.
        </p>
        <div className="mt-3.5 flex items-center gap-2">
          <Button
            className="h-7 rounded-lg bg-white px-3 text-xs text-black hover:bg-white/90"
            size="sm"
          >
            Get Started
          </Button>
          <Button
            className="h-7 rounded-lg border-white/20 bg-transparent px-3 text-xs text-white"
            size="sm"
            variant="outline"
          >
            View Demo
          ...
          <div className="flex items-end justify-between">
            <div>
              ...
              <p className="font-bold text-base text-white leading-tight">
                $24,880 <span className="font-medium text-xs text-emerald-400">+7.5%</span>
              </p>
            </div>
          </div>
          ...
          <div className="mt-3 grid grid-cols-4 gap-1.5">
            {STATS.map((stat) => (
              ...
              >
                ...
                <p className="font-semibold text-xs text-white leading-tight">{stat.value}</p>
              </div>
            ))}
          </div>
        </CardContent>
