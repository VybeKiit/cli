    ...
    <div className="flex h-full bg-[#0a0d13] text-[var(--text-soft)]">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex items-center gap-2 border-white/8 border-b px-3 py-2.5">
          ...
          <span className="rounded bg-[var(--green-soft)] px-1.5 py-0.5 text-[8px] text-[var(--green)]">
            Live
          </span>
        ...
        <ul className="flex min-h-0 flex-1 flex-col gap-1.5 overflow-hidden px-3 pb-3">
          {TICKETS.map((ticket) => (
            <li
              className="flex items-start gap-2.5 rounded-lg border border-white/6 bg-white/[0.03] px-2.5 py-2"
              key={ticket.title}
            >
