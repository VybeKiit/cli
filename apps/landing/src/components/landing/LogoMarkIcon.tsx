 ...
 * Paths are site-relative; {@link cdnAssetUrl} rewrites them to the R2 public origin
 * (`Cache-Control: max-age=86400`). Re-upload with `pnpm upload-assets:r2`.
...
export const LOGO_MARK_RASTERS: Record<string, string> = {
  ...
  // Fixed-size cloud raster (CodexMark SVG uses width/height 100% elsewhere and blows up in marquees)
  codex: '/brand-marks/codex.webp',
  ...
  // AI coding agents (official marks: kiro.dev, app.devin.ai / devin.ai)
  ...
  playwright: '/brand-marks/playwright.webp',
...
const customSvgMarks: Record<string, LogoMarkIconData> = {
  ...
  // simple-icons dropped amazonaws; keep official smile wordmark path.
  amazonaws: {
    title: 'AWS',
    slug: 'amazonaws',
    viewBox: '0 0 24 24',
...
export const LOGO_MARK_ICONS: Record<string, LogoMarkIconData> = {
  ...
  amazonaws: customSvgMarks.amazonaws!,
...
export const LogoMarkIcon = ({ slug, className, mono = false }: LogoMarkIconProps) => {
  // Prefer the fixed-size codex raster in marquees so `.codex-mark { width/height: 100% }` cannot expand.
  // Callers that need the animated SVG can still import CodexMark directly.
  if (slug === 'codex' && mono) {
    return (
      <span
        aria-hidden="true"
        className={cn(
          'logo-mark-icon inline-flex shrink-0 items-center justify-center overflow-hidden',
          className,
        )}
        data-mark-slug="codex"
      >
        <CodexMark className="!h-full !w-full max-h-full max-w-full" />
      </span>
    );
  }
  ...
  if (rasterSrc) {
    return (
      <img
        ...
        // React preload steals mobile bandwidth from FCP/LCP on slow networks.
        draggable={false}
        height={24}
        loading="lazy"
  ...
  return (
    <svg
      aria-hidden="true"
      className={cn('logo-mark-icon', className)}
      fill="currentColor"
      role="img"
