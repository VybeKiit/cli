  ...
  >
    <Card className="mb-[18px] h-[106px] gap-0 border-black/6 bg-white py-0 shadow-none">
      <CardContent className="flex h-full items-center justify-between p-5">
        <div>
          <p className="font-semibold text-lg text-[var(--light-text)]">Provider</p>
          <div className="mt-2 flex items-center gap-2">
            <LogoMarkIcon className="h-4 w-4" slug="stripe" />
            <span className="font-medium text-base text-[var(--light-text)]">Stripe</span>
          </div>
        </div>
        <Badge className="border-[var(--blue)]/20 bg-[var(--blue)]/10 px-3 py-1.5 text-sm text-[var(--blue-strong)]">
          Connected
        </Badge>
      </CardContent>
    </Card>
    ...
    <div className="grid items-start gap-4 sm:grid-cols-2">
      <Card className="min-h-[258px] gap-0 border-black/6 bg-white py-0 shadow-none">
        <CardContent className="p-5">
          <p className="text-base text-[var(--light-muted)]">Today</p>
          <div className="mt-0.5 flex items-baseline gap-2">
            <p className="font-bold text-3xl text-[var(--light-text)] tracking-tight">$2,430.00</p>
            <span className="font-medium text-sm text-emerald-600">+12.5%</span>
          </div>
          <p className="text-sm text-[var(--light-muted)]">vs yesterday</p>
      ...
      <Card className="min-h-[258px] gap-0 border-black/6 bg-white py-0 shadow-none">
        <CardContent className="p-5">
          <p className="mb-4 font-semibold text-base text-[var(--light-text)]">
            Recent Transactions
          </p>
          ...
          <ul className="space-y-4">
            {TRANSACTIONS.map((tx) => (
              <li className="flex items-center justify-between" key={tx.plan}>
                <div>
                  <p className="font-semibold text-base text-[var(--light-text)] leading-tight">
                    {tx.amount}
                  </p>
                  <p className="text-sm text-[var(--light-muted)] leading-tight">{tx.plan}</p>
                </div>
                <span className="text-sm text-[var(--light-muted)]">{tx.time}</span>
              </li>
            ))}
          </ul>
        </CardContent>
