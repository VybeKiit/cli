...
import { BRAND, SUPPORT } from '@/data/site';
import { useLandingLocale } from '@/i18n/LocaleProvider';

/** Calendar year for the copyright line (re-evaluated each render). */
    ...
    <footer className="border-border/60 border-t">
      <div className="mx-auto flex max-w-5xl flex-col gap-6 px-6 py-12 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-sm text-muted-foreground text-sm">{messages.brand.tagline}</p>
          <p className="mt-3 text-muted-foreground text-sm">
            © {COPYRIGHT_YEAR} {BRAND.name}. {messages.footer.rights}
          </p>
        </div>
        <div className="flex flex-col items-start gap-4 sm:items-end">
          <nav
            aria-label={messages.footer.legal}
