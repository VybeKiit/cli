...
/** A note on the customer timeline (mirrors customer_notes). */
type Note = {
  readonly id: string;
      ...
      <main className="mx-auto max-w-5xl px-4 py-10">
          ...
          <TabsContent className="mt-4 space-y-4" value="notes">
            ...
            {notes.length === 0 ? (
              <div className="flex flex-col items-center rounded-lg border border-dashed px-4 py-12 text-center">
                <StickyNote aria-hidden="true" className="h-8 w-8 text-muted-foreground" />
                <h2 className="mt-3 font-semibold">No notes yet</h2>
        ...
        <details className="mt-8 rounded-lg border bg-card p-4 text-sm">
          ...
          <div className="mt-3 space-y-2 text-muted-foreground">
            <p>
              Fully interactive with local state — tabs switch sections, and the notes form appends
              a live timeline entry (or clears to the empty state). To make it real:
            </p>
...
const Frame = ({ children }: { readonly children: ReactNode }) => (
  <DemoThemeRandomizer>
    <DemoTransitionStage defaultTransition="fade" title="Customer detail motion pass">
