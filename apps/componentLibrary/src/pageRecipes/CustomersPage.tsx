              ...
              return (
                ...
                >
                  ...
                  <TableCell className="text-right font-medium tabular-nums">
                    {customer.mrrCents === 0 ? '—' : usd(customer.mrrCents)}
                  </TableCell>
                </TableRow>
              );
            })}
      ...
      <main className="mx-auto max-w-6xl px-4 py-10">
        ...
        <details className="mt-8 rounded-lg border bg-card p-4 text-sm">
          ...
          <div className="mt-3 space-y-2 text-muted-foreground">
            <p>
              Fully interactive with local state — search and status filters recompute the table,
              and selecting a row pins the profile strip. To make it real:
            </p>
            <ol className="list-decimal space-y-1 pl-5">
              <li>
                Run <code>vybekiit apply-preset customers</code> to create the{' '}
                <code>customers</code> and <code>customer_notes</code> tables.
              </li>
              <li>
              ...
              <li>
                Wire row open to <code>/customers/[id]</code> (the customer detail recipe) and
                persist status changes with <code>PATCH /api/customers/:id</code>.
              </li>
            </ol>
          </div>
        </details>
...
const Frame = ({ children }: { readonly children: ReactNode }) => (
  <DemoThemeRandomizer>
    <DemoTransitionStage defaultTransition="fade" title="Customers motion pass">
      <div className="min-h-screen bg-background text-foreground">{children}</div>
    </DemoTransitionStage>
  </DemoThemeRandomizer>
);

/** One KPI tile above the customer table. */
const Kpi = ({
  icon,
  label,
  value,
...
const EmptyFilterState = ({ onClear }: { readonly onClear: () => void }) => (
