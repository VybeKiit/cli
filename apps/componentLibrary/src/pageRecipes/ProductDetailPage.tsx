      ...
      <Frame>
        <section className="mx-auto max-w-md px-4 py-24">
          <Alert variant="destructive">
            ...
            <AlertTitle>Product could not load</AlertTitle>
            <AlertDescription>
              Details, variants, and related products failed to load. Retry when the products API is
              reachable.
            </AlertDescription>
          </Alert>
          <div className="mt-6 flex justify-center">
            <Button onClick={() => setLoadError(false)} type="button">
              Retry
            </Button>
          </div>
        </section>
            ...
            {RELATED.map((item) => (
              <li key={item.id}>
                <Card className="h-full">
                  ...
                  <CardContent>
                    <p className="font-medium text-sm tabular-nums">{usd(item.priceCents)}</p>
                  </CardContent>
                </Card>
              </li>
            ))}
        ...
        <details className="mt-8 rounded-lg border bg-card p-4 text-sm">
          ...
          <div className="mt-3 space-y-2 text-muted-foreground">
            <p>
              ...
              it. To wire a real product detail route:
            </p>
            <ol className="list-decimal space-y-1 pl-5">
              ...
              <li>
                ...
                <code>
                  {'{ cart_id, product_id, variant_id, name, unit_price_cents, quantity }'}
                </code>
                .
              </li>
              <li>
...
const Frame = ({ children }: { readonly children: ReactNode }) => (
  <DemoThemeRandomizer>
    <DemoTransitionStage defaultTransition="slide" title="Product detail motion pass">
...
const AddToCartLabel = ({
  ...
  return (
    <>
      <ShoppingCart aria-hidden="true" className="h-4 w-4" /> Add to cart · {usd(lineTotal)}
    </>
  );
};
