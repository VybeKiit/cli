...
const stepStatus = (id: number, current: number): 'done' | 'current' | 'todo' => {
  if (id < current) {
    return 'done';
  }
  if (id === current) {
      ...
      <main className="mx-auto max-w-2xl px-4 py-10">
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      {TEAM_SIZES.map((option) => (
                        <Label
                          className={cn(
                            'flex flex-1 cursor-pointer items-center justify-center rounded-md px-3 py-1.5 text-center font-medium text-sm transition-colors',
                            teamSize === option.id
                              ? 'bg-background shadow-sm'
                              : 'text-muted-foreground',
                          )}
                          htmlFor={`${teamSizeLabelId}-${option.id}`}
                          key={option.id}
                        >
                          <RadioGroupItem
                            className="sr-only"
                            id={`${teamSizeLabelId}-${option.id}`}
                            value={option.id}
                          />
                          {option.label}
                        </Label>
        ...
        <details className="mt-8 rounded-lg border bg-card p-4 text-sm">
          ...
          <div className="mt-3 space-y-2 text-muted-foreground">
            <p>
              Fully interactive with local state — the slug derives from the name live, Next is
              gated on validation, and invites validate before they're added. To make it real:
            </p>
            <ol className="list-decimal space-y-1 pl-5">
              <li>
                Persist each step (or the whole payload on Finish) with{' '}
                <code>POST /api/onboarding</code> writing to your DB / D1; check the{' '}
                <code>slug</code> is free before you commit it.
              </li>
              <li>
              ...
              <li>
                Swap the <code>setTimeout</code> in <code>finish</code> for the real request and
                keep the <code>provisioning</code> → <code>done</code> states as-is.
              </li>
            </ol>
          </div>
        </details>
...
const Frame = ({ children }: { readonly children: ReactNode }) => (
  <DemoThemeRandomizer>
    <DemoTransitionStage defaultTransition="slide" title="Onboarding motion pass">
...
const Stepper = ({ current }: { readonly current: number }) => (
