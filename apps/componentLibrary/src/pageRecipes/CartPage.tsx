    ...
    return (
      <Frame>
        <section className="mx-auto flex max-w-md flex-col items-center px-4 py-24 text-center">
          ...
          <h1 className="mt-6 font-bold text-2xl tracking-tight">Loading cart…</h1>
          <p className="mt-2 text-muted-foreground text-sm">Syncing line items and totals.</p>
        </section>
      </Frame>
    );
  }
      ...
      <Frame>
        <section className="mx-auto max-w-md px-4 py-24">
          <Alert variant="destructive">
            ...
            <AlertTitle>Cart could not load</AlertTitle>
            <AlertDescription>
              Line items failed to load. Retry once the cart API is reachable.
            </AlertDescription>
          </Alert>
          <div className="mt-6 flex justify-center">
            <Button onClick={reloadCart} type="button">
              Retry
            </Button>
          </div>
        </section>
            ...
            <CardContent className="space-y-4">
              ...
              <dl aria-atomic="true" aria-live="polite" className="space-y-1.5 text-sm">
                ...
                {hasPhysical ? (
                  <Row
                    label="Shipping"
                    value={totals.freeShipping ? 'Free' : usd(totals.shippingCost)}
                  />
                ) : null}
              ...
              {hasPhysical && !totals.freeShipping ? (
                <p className="text-muted-foreground text-xs">
                  Add {usd(FREE_SHIPPING_THRESHOLD - (totals.subtotal - totals.discount))} more for
                  free shipping.
                </p>
              ) : null}
        ...
        <details className="mt-8 rounded-lg border bg-card p-4 text-sm">
          ...
          <div className="mt-3 space-y-2 text-muted-foreground">
            <p>
              ...
              it. To wire a real multi-device cart:
            </p>
...
const Frame = ({ children }: { readonly children: ReactNode }) => (
  <DemoThemeRandomizer>
    <DemoTransitionStage defaultTransition="scale" title="Cart motion pass">
