/**
 * Next.js config for the extension landing page (apps/extensionLandingPage).
 *
 * Static marketing site — no Sentry, D1, or payments. Mirrors `apps/landing`'s
 * root-`.env` loader, `externalDir` + `optimizePackageImports`, R2 image
 * `remotePatterns`, and the webpack block that lets this app import reused
 * `templates/web` components via the `@vybekiit-template-web` alias (rewriting
 * their internal `@/…` imports to the template `src`). Also transpiles the
 * shared `@vybekiit/ui` package and enables the Cloudflare dev bindings.
...
const nextConfig = {
  ...
  experimental: {
    externalDir: true,
    // Tree-shake barrel imports (lucide, simple-icons) so first-load stays lean.
