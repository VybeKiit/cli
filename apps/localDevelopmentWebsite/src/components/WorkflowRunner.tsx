...
export const WorkflowRunner = ({ compact = false }: WorkflowRunnerProps) => {
  ...
  // Dynamic workflow detection: watch the latest user message
  useEffect(() => {
  ...
  // Fallback: set default workflow if nothing detected
  useEffect(() => {
    if (!workflow) {
      setWorkflow(saasWorkflow);
    }
  ...
  return (
    <div className={cn('space-y-4', compact && 'space-y-3')}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            {!compact && <p className="mt-0.5 text-xs text-zinc-400">{workflow.tagline}</p>}
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Button
      ...
      {(isRunning || progress > 0) && (
        <Progress
          value={progress}
          className="h-1.5 bg-zinc-800 [&>div]:bg-gradient-to-r [&>div]:from-vybe-600 [&>div]:to-emerald-500"
        />
      )}
      ...
      {allStepsDone && collapsed && (
        <button
          ...
          className="w-full rounded-lg border border-emerald-500/20 bg-emerald-950/10 px-3 py-2 text-left text-xs text-emerald-400 transition-colors hover:bg-emerald-950/20"
        >
