...
/** String that parses as a URL — replaces Zod's `.url()` refinement. */
const UrlString = Schema.String.pipe(
  Schema.filter((value) => URL.canParse(value), { message: () => 'Expected a valid URL.' }),
);

/**
 * Effect `Schema` for `extensions/<name>/cws-listing.ts` — the per-extension source
 * of truth for everything the Chrome Web Store dev-console pushes for this
 * package's verbs.
 *
 * Why this lives in an Effect `Schema` rather than a plain TS type:
 *
 ...
 * - The TS type is derived (`CwsListing = Schema.Schema.Type<typeof CwsListingSchema>`)
 ...
 */
export const CwsListingSchema = Schema.Struct({
  /**
  ...
  distribution: Schema.Struct({
     ...
     */
    payments: Schema.Literal('Free of charge', 'Contains in-app purchases'),
     ...
     */
    regions: Schema.Record({ key: Schema.String, value: Schema.Boolean }),
  ...
  listing: Schema.Struct({
     ...
     */
    description: Schema.String.pipe(Schema.minLength(1), Schema.maxLength(16_000)),
     ...
     */
    globalPromoVideo: Schema.optional(UrlString),
     ...
     */
    homepageUrl: Schema.optional(UrlString),
     ...
     */
    officialUrl: Schema.optional(Schema.String.pipe(Schema.minLength(1))),
     ...
     */
    promoTileMarquee: Schema.optional(Schema.String.pipe(Schema.minLength(1))),
     ...
     */
    promoTileSmall: Schema.optional(Schema.String.pipe(Schema.minLength(1))),
     ...
     */
    screenshots: Schema.optionalWith(
      Schema.Array(Schema.String.pipe(Schema.minLength(1))).pipe(Schema.maxItems(10)),
      { default: () => [] },
    ),
  ...
  package: Schema.Struct({
     ...
     */
    verifiedUploadsOptIn: Schema.Boolean,
  }),
  ...
  privacy: Schema.Struct({
     ...
     * these are true.
     */
    dataUseDisclosure: Schema.optional(
      Schema.Struct({
        collectsActivity: Schema.Boolean,
        collectsAuth: Schema.Boolean,
        collectsFinancial: Schema.Boolean,
        collectsHealth: Schema.Boolean,
        collectsLocation: Schema.Boolean,
        collectsPersonalCommunications: Schema.Boolean,
        collectsPii: Schema.Boolean,
        collectsWebHistory: Schema.Boolean,
        collectsWebsiteContent: Schema.Boolean,
      }),
    ),
     ...
     * actual code.
     */
    permissionsJustification: Schema.Record({
      key: Schema.String,
     ...
     */
    remoteCodeJustification: Schema.optional(
      Schema.String.pipe(Schema.minLength(1), Schema.maxLength(1000)),
    ),
     ...
     */
    singlePurpose: Schema.String.pipe(Schema.minLength(1), Schema.maxLength(1000)),
  ...
  status: Schema.Struct({
     ...
     */
    review: Schema.Literal('draft', 'in_review', 'published', 'rejected', 'unknown'),
 ...
 */
export type CwsListing = Schema.Schema.Type<typeof CwsListingSchema>;
