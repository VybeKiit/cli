...
Client cache and UI prefs for web, mobile, and extension.
...
`resolveClientState()` wires TanStack Query defaults and Zustand/MMKV UI store factories per surface.
...
- **Do not edit** node_modules/@vybekiit/client-state — fix bugs upstream or bump the package version.
- **Entry point:** `resolveClientState`, `createQueryClient`, `createUiStore`, `./mobile`
- **Config:** `CLIENT_STATE_PERSIST`, `CLIENT_STATE_QUERY_STALE_SECONDS`
- **Related skills:** client-state-vybekiit.md
...
- TanStack Query 5.101.x, Zustand scaffold.
- Server-side Redis — not a buyer path.
...
```bash
npm update @vybekiit/client-state
```
