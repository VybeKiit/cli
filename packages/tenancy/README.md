...
Teams and workspaces — org accounts without building from scratch.
...
Organization membership primitives. Better Auth + db default.
...
- **Do not edit** node_modules/@vybekiit/tenancy — fix bugs upstream or bump the package version.
- **Entry point:** `resolveTenancyProvider`, `TenancyProvider`, `OrgMember`
- **Config:** `TENANCY_PROVIDER` (default `better-auth`)
- **Related skills:** `add-teams`
...
- Org CRUD, membership roles interface.
- No team settings UI — agent adds screens on request.
