...
Cookie banner and data-export hooks — stay compliant without legal jargon.
...
Cookie consent config and export hooks your agent enables when you add analytics or store user data.
...
- **Do not edit** node_modules/@vybekiit/compliance — fix bugs upstream or bump the package version.
- **Entry point:** `resolveComplianceProvider`, `createLocalCompliance`, `CookieConsentConfig`
- **Config:** `COMPLIANCE_PROVIDER` (default `local`), `COOKIE_CONSENT_ENABLED` (default `on`)
- **Related skills:** `harden`, privacy pages
...
- Consent config, export hook interface.
- Not legal advice — templates ship starter privacy/terms copy.
...
```bash
npm update @vybekiit/compliance
```
