...
Send transactional email — welcome messages, receipts, alerts.
...
Provider-agnostic send API. Cloudflare Email is default; SES and Resend available via env.
...
- **Do not edit** node_modules/@vybekiit/email — fix bugs upstream or bump the package version.
- **Entry point:** `resolveEmailProvider`, `createCloudflareEmail`, `createSesEmail`
- **Config:** `EMAIL_PROVIDER` (default `cloudflare`)
- **Related skills:** `setup-email`, `add-notifications`
...
- Send transactional email, typed params.
- No email templates UI — copy lives in your app.
