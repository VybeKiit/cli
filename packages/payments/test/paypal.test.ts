...
describe('paypal webhook mapper', () => {
  it('reads github username from a purchase unit on an order event', async () => {
  ...
  it('reads github username from custom_id and flags refunds on a capture event', async () => {
    const result = await Effect.runPromise(
      mapPayPalEvent({
  ...
  it('fails when the PayPal resource id is missing', async () => {
