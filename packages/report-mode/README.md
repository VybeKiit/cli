...
Dev-only Report Mode for vibe coders: structured reports, console error buffer, native assistant deeplink. Never ships to production builds.
...
- **Do not edit** node_modules/@vybekiit/report-mode — fix bugs upstream or bump the package version.
- **Entry point:** `formatReportPrompt`, `buildAssistantDeepLink`, `ConsoleErrorBuffer`, `REPORT_MODE_HOTKEY_LABEL`
- **Config:** None — dev-only, tree-shaken from prod.
- **Related skills:** Report Mode section in agent layer docs
...
- Bug report formatting, deeplink handoff.
- Not production error tracking — use `@vybekiit/observability`.
...
```bash
npm update @vybekiit/report-mode
```
