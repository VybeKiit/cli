...
Background tasks and scheduled jobs — reminders, cleanups, digests.
...
Jobs adapter for cron and queue workloads. Cloudflare Cron/Queues default.
...
- **Do not edit** node_modules/@vybekiit/jobs — fix bugs upstream or bump the package version.
- **Entry point:** `resolveJobsProvider`, `JobsProvider`, `JobPayload`
- **Config:** `JOBS_PROVIDER` (default `cloudflare`)
- **Related skills:** Feature skills that schedule work
...
- Enqueue, schedule, provider binding.
- No job admin UI.
