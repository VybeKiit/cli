...
const searchUiComponentsInputSchema = {
  ...
  properties: {
    ...
    fields: {
      type: 'string',
      enum: ['slim', 'full'],
      description: 'slim (default): source/name/category/tags/score; full: includes paths+deps',
    },
  },
  required: ['query'],
...
const tools = [
  {
    ...
    description:
      'Fuzzy-search mirrored UI components. Returns a page envelope (items, total, hasMore, nextCursor). Prefer fields=slim to save context; use get_ui_component for full detail.',
